...
async function sendToClaude(
	...
	{ model = "claude-3-7-sonnet-20250219", maxTokens = 2048 } = {}
) {
	try {
...
async function verifyEmailSync(email: string) {
	const url = new URL(
		"https://api.apify.com/v2/acts/fatihtahta~email-verifier-free-to-use/run-sync-get-dataset-items"
	);
	...
	if (!resp.ok) {
		...
		throw new Error(
			`Apify run-sync error: ${resp.status} ${resp.statusText}: ${text}`
		);
	}
...
export async function processJobsForUser(user_id: string) {
	...
	const { error: updateEnrichError } = await supabase
		.from("leads")
		.update(
			enrichFailed
				? {
	...
	if (emails.length > 0) {
		try {
			...
			if (verifyEmailData?.[0]?.status === "valid") {
	...
	const { error: updateVerifyEmailError } = await supabase
		.from("leads")
		.update({
