...
export class ProxyLogsService {
  ...
  async getProxyLogsPaginated(filters: ProxyLogFilters = {}): Promise<PaginatedProxyLogResponse> {
    ...
    if (filters.search) {
      ...
      query = query.or(`proxy_host.ilike.${searchTerm},error.ilike.${searchTerm}`);
      countQuery = countQuery.or(`proxy_host.ilike.${searchTerm},error.ilike.${searchTerm}`);
    }
  ...
  async logProxyOperation(params: LogProxyOperationParams): Promise<ProxyLog> {
    const { proxy_host, proxy_port, proxy_ip, startTime, endTime, status, error, web_source, web_url } = params;
