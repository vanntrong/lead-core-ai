...
import { PaginatedProxyLogResponse, ProxyLog, ProxyLogFilters, ProxyLogStats } from "@/types/proxy_log";
...
export class ProxyLogsAdminService {
  ...
  async getProxyLogsPaginated(filters: ProxyLogFilters = {}): Promise<PaginatedProxyLogResponse> {
    const supabase = await this.getSupabaseClient();
    const page = filters.page ?? 1;
    const limit = filters.limit ?? 20;
    const offset = (page - 1) * limit;
    let query = supabase.from('proxy_logs').select('*');
    let countQuery = supabase.from('proxy_logs').select('*', { count: 'exact', head: true });
    if (filters.status) {
    ...
    if (countError) {
      ...
      throw new Error(`Failed to fetch proxy logs count: ${countError.message}`);
    }
  ...
  async getRecentLogsByHost(proxy_host: string, limit: number = 10): Promise<ProxyLog[]> {
  ...
  async getFailedLogs(filters?: { proxy_host?: string; since?: string }): Promise<ProxyLog[]> {
