...
import { CreateScraperLogData, PaginatedScraperLogResponse, ScraperLog, ScraperLogFilters, ScraperLogStats, ScraperSource, UpdateScraperLogData } from "@/types/scraper_log";
...
export class ScraperLogsService {
  ...
  async getScraperLogsPaginated(filters: ScraperLogFilters = {}): Promise<PaginatedScraperLogResponse> {
    const supabase = await this.getSupabaseClient();
    ...
    let countQuery = supabase.from('scraper_logs').select('*', { count: 'exact', head: true });
    ...
    if (error) {
      ...
      throw new Error(`Failed to fetch paginated scraper logs: ${error.message}`);
    }

    if (countError) {
      console.error("Error fetching scraper logs count:", countError);
      throw new Error(`Failed to fetch scraper logs count: ${countError.message}`);
    }
  ...
  async getScraperLogStats(): Promise<ScraperLogStats> {
    ...
    const average_duration = logsWithDuration.length > 0
    ...
    const source_breakdown = Object.entries(sourceMap).map(([source, stats]) => ({
      source,
      count: stats.total,
  ...
  async logScrapingOperation(params: LogScrapingOperationParams): Promise<ScraperLog> {
  ...
  async getRecentLogsBySource(source: ScraperSource, limit: number = 10): Promise<ScraperLog[]> {
  ...
  async getFailedLogs(filters?: { source?: ScraperSource; since?: string }): Promise<ScraperLog[]> {
