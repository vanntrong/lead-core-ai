...
export class ProxyHealCheckLogsAdminService {
	...
	async createLog(
		data: Partial<ProxyHealCheckLog>
	): Promise<ProxyHealCheckLog> {
		...
		if (error) {
			...
			throw new Error(
				`Failed to create proxy heal check log: ${error.message}`
			);
		}
		return newLog;
	}
	...
	async updateLog(
		data: Partial<ProxyHealCheckLog> & { id: string }
	): Promise<ProxyHealCheckLog> {
		const supabase = await this.getSupabaseClient();
		...
		if (error) {
			...
			throw new Error(
				`Failed to update proxy heal check log: ${error.message}`
			);
		}
		return updatedLog;
	}
	...
	async deleteLog(id: string): Promise<void> {
		...
		if (error) {
			...
			throw new Error(
				`Failed to delete proxy heal check log: ${error.message}`
			);
		}
	}
	...
	async getLogsPaginated(
	...
	): Promise<PaginatedProxyHealCheckLogResponse> {
		const supabase = await this.getSupabaseClient();
		...
		if (error) {
			console.error(
				"Error fetching paginated proxy heal check logs (admin):",
				error
			);
			throw new Error(
				`Failed to fetch paginated proxy heal check logs: ${error.message}`
			);
		}
		if (countError) {
			console.error(
				"Error fetching proxy heal check logs count (admin):",
				countError
			);
			throw new Error(
				`Failed to fetch proxy heal check logs count: ${countError.message}`
			);
		}
	...
	async getLogs(
		...
		if (error) {
			...
			throw new Error(
				`Failed to fetch proxy heal check logs: ${error.message}`
			);
		}
		return data ?? [];
	}
	...
	async getLogStats(): Promise<{
		...
		if (error) {
			console.error(
				"Error fetching proxy heal check log stats (admin):",
				error
			);
			throw new Error(
				`Failed to fetch proxy heal check log stats: ${error.message}`
			);
		}
	...
	async logProxyHealCheckOperation(
		params: LogProxyHealCheckOperationParams
	): Promise<ProxyHealCheckLog> {
		const {
			proxy_host,
			proxy_port,
...
export const proxyHealCheckLogsAdminService =
	new ProxyHealCheckLogsAdminService();
