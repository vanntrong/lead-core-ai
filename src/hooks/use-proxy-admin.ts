...
import type { Proxy, ProxyFilters } from "@/types/proxy";
import {
	keepPreviousData,
	useMutation,
	useQuery,
...
export const proxyAdminKeys = {
	...
	list: (filters?: ProxyFilters) =>
		[...proxyAdminKeys.lists(), filters] as const,
	paginatedLists: () => [...proxyAdminKeys.all, "paginatedList"] as const,
	paginatedList: (filters?: ProxyFilters) =>
		[...proxyAdminKeys.paginatedLists(), filters] as const,
	...
	return useMutation({
		...
		onSuccess: (updatedProxy: Proxy) => {
			queryClient.setQueryData(
				proxyAdminKeys.detail(updatedProxy.id),
				updatedProxy
			);
			queryClient.invalidateQueries({ queryKey: proxyAdminKeys.lists() });
			queryClient.invalidateQueries({
				queryKey: proxyAdminKeys.paginatedLists(),
			});
		},
		onError: (error) => {
