...
export const scraperLogAdminKeys = {
  ...
  list: (filters?: ScraperLogFilters) => [...scraperLogAdminKeys.lists(), filters] as const,
  paginatedLists: () => [...scraperLogAdminKeys.all, "paginatedList"] as const,
  paginatedList: (filters?: ScraperLogFilters) => [...scraperLogAdminKeys.paginatedLists(), filters] as const,
...
export function useCreateScraperLogAdmin() {
  ...
  return useMutation({
    mutationFn: (data: CreateScraperLogData) => createScraperLogAdminAction(data),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: scraperLogAdminKeys.lists() });
      queryClient.invalidateQueries({ queryKey: scraperLogAdminKeys.paginatedLists() });
...
export function useUpdateScraperLogAdmin() {
  ...
  return useMutation({
    ...
    onSuccess: (updatedLog: ScraperLog) => {
      queryClient.setQueryData(scraperLogAdminKeys.detail(updatedLog.id), updatedLog);
      queryClient.invalidateQueries({ queryKey: scraperLogAdminKeys.lists() });
      queryClient.invalidateQueries({ queryKey: scraperLogAdminKeys.paginatedLists() });
    },
    onError: (error) => {
...
export function useDeleteScraperLogAdmin() {
  ...
  return useMutation({
    ...
    onSuccess: (_, deletedId) => {
      queryClient.removeQueries({ queryKey: scraperLogAdminKeys.detail(deletedId) });
      queryClient.invalidateQueries({ queryKey: scraperLogAdminKeys.lists() });
      queryClient.invalidateQueries({ queryKey: scraperLogAdminKeys.paginatedLists() });
    },
    onError: (error) => {
