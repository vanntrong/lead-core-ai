...
export const proxyHealCheckLogAdminKeys = {
  ...
  list: (filters?: ProxyHealCheckLogFilters) => [...proxyHealCheckLogAdminKeys.lists(), filters] as const,
  paginatedLists: () => [...proxyHealCheckLogAdminKeys.all, "paginatedList"] as const,
  paginatedList: (filters?: ProxyHealCheckLogFilters) => [...proxyHealCheckLogAdminKeys.paginatedLists(), filters] as const,
  details: () => [...proxyHealCheckLogAdminKeys.all, "detail"] as const,
  detail: (id: string) => [...proxyHealCheckLogAdminKeys.details(), id] as const,
...
export function useProxyHealCheckLogsPaginatedAdmin(filters?: ProxyHealCheckLogFilters) {
  return useQuery({
...
export function useCreateProxyHealCheckLogAdmin() {
  ...
  return useMutation({
    mutationFn: (data: CreateProxyHealCheckLogData) => createProxyHealCheckLogAdminAction(data),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: proxyHealCheckLogAdminKeys.lists() });
      queryClient.invalidateQueries({ queryKey: proxyHealCheckLogAdminKeys.paginatedLists() });
      queryClient.invalidateQueries({ queryKey: proxyHealCheckLogAdminKeys.stats() });
    },
    onError: (error) => {
...
export function useUpdateProxyHealCheckLogAdmin() {
  ...
  return useMutation({
    ...
    onSuccess: (updatedLog: ProxyHealCheckLog) => {
      queryClient.setQueryData(proxyHealCheckLogAdminKeys.detail(updatedLog.id), updatedLog);
      queryClient.invalidateQueries({ queryKey: proxyHealCheckLogAdminKeys.lists() });
      queryClient.invalidateQueries({ queryKey: proxyHealCheckLogAdminKeys.paginatedLists() });
    },
    onError: (error) => {
...
export function useDeleteProxyHealCheckLogAdmin() {
  ...
  return useMutation({
    ...
    onSuccess: (_, deletedId) => {
      queryClient.removeQueries({ queryKey: proxyHealCheckLogAdminKeys.detail(deletedId) });
      queryClient.invalidateQueries({ queryKey: proxyHealCheckLogAdminKeys.lists() });
      queryClient.invalidateQueries({ queryKey: proxyHealCheckLogAdminKeys.paginatedLists() });
    },
    onError: (error) => {
