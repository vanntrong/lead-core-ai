...
export const proxyLogAdminKeys = {
  ...
  list: (filters?: ProxyLogFilters) => [...proxyLogAdminKeys.lists(), filters] as const,
  paginatedLists: () => [...proxyLogAdminKeys.all, "paginatedList"] as const,
  paginatedList: (filters?: ProxyLogFilters) => [...proxyLogAdminKeys.paginatedLists(), filters] as const,
...
export function useCreateProxyLogAdmin() {
  ...
  return useMutation({
    ...
    onSuccess: () => {
      ...
      queryClient.invalidateQueries({ queryKey: proxyLogAdminKeys.paginatedLists() });
...
export function useUpdateProxyLogAdmin() {
  ...
  return useMutation({
    ...
    onSuccess: (updatedLog: ProxyLog) => {
      queryClient.setQueryData(proxyLogAdminKeys.detail(updatedLog.id), updatedLog);
      queryClient.invalidateQueries({ queryKey: proxyLogAdminKeys.lists() });
      queryClient.invalidateQueries({ queryKey: proxyLogAdminKeys.paginatedLists() });
    },
    onError: (error) => {
...
export function useDeleteProxyLogAdmin() {
  ...
  return useMutation({
    ...
    onSuccess: (_, deletedId) => {
      queryClient.removeQueries({ queryKey: proxyLogAdminKeys.detail(deletedId) });
      queryClient.invalidateQueries({ queryKey: proxyLogAdminKeys.lists() });
      queryClient.invalidateQueries({ queryKey: proxyLogAdminKeys.paginatedLists() });
    },
    onError: (error) => {
