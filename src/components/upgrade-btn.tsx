...
export function UpgradeButton({ currentPlan, title, className }: UpgradeButtonProps) {
  ...
  const nextPlanFeatures = pricingPlans.find(p => p.tier === nextPlan)?.features?.filter(f => !f.startsWith("Empty")) || [];
        ...
        <DialogContent onInteractOutside={e => e.preventDefault()}>
          <DialogHeader>
            ...
            <DialogDescription asChild>
              <div className="bg-gradient-to-br from-indigo-50/80 to-purple-50/80 rounded-xl p-4 border border-indigo-10 mt-4">
                <div className="flex items-center gap-2 mb-4">
                  <Crown className="h-4 w-4 text-indigo-600" />
                  <span className="font-semibold text-gray-900 text-sm">
                    Upgrade to {pricingPlans.find(p => p.tier === nextPlan)?.name}
                  </span>
                </div>
                <div className="space-y-3">
                  {nextPlanFeatures.map(feature => (
                    <div key={feature} className="flex items-start gap-3 group">
                      <div className="w-5 h-5 bg-gradient-to-br from-indigo-500 to-purple-600 rounded-full flex items-center justify-center mt-0.5 flex-shrink-0">
                ...
                <div className="mt-4 pt-3 border-t border-indigo-100">
                  <div className="flex items-center justify-between">
                    ...
                    <span className="font-bold text-lg text-gray-900">
                      ${(pricingPlans.find(p => p.tier === nextPlan)?.priceMonthly || 0) / 100}/mo
                    </span>
                  </div>
                </div>
              </div>
