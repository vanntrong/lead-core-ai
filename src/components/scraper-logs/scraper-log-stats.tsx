...
import type { ScraperLogStats } from "@/types/scraper_log";
import { formatDuration } from "@/utils/helper";
import { leadSourceColorConfig } from "@/constants/saas-source";
import { BarChart3, AlertCircle, Timer, CheckCircle2 } from "lucide-react";
...
export function ScraperLogStatsCards({
	stats,
	isLoading,
}: ScraperLogStatsProps) {
	if (isLoading) {
		return (
			...
			<Card className="border-gray-200 bg-white p-4 shadow-sm transition-shadow hover:shadow-md">
				<div className="flex items-center justify-between">
					<div>
						...
						<p className="font-bold text-gray-900 text-xl">{stats?.success}</p>
					</div>
			...
			<Card className="border-gray-200 bg-white p-4 shadow-sm transition-shadow hover:shadow-md">
				<div className="flex items-center justify-between">
					<div>
						...
						<p className="font-bold text-gray-900 text-xl">{stats?.failed}</p>
					</div>
					...
					<div>
						...
						<p className="font-bold text-gray-900 text-xl">
							{stats?.average_duration
								? `${formatDuration(stats.average_duration)} s`
								: "-"}
						</p>
					</div>
						...
						<p
							...
							title={(() => {
								if (stats?.top_sources && stats.top_sources.length > 0) {
									...
									const label =
										leadSourceColorConfig[topSource]?.label || topSource;
							...
							{(() => {
								if (stats?.top_sources && stats.top_sources.length > 0) {
									...
									return (
										<>
											<span
												className="inline-block max-w-[70px] truncate align-bottom"
												title={label}
											>
												{label}
											</span>
